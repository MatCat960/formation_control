...
namespace formation_control
{
    ...
    FormationController::FormationController(double fow_angle, double min_distance, double max_distance, int num_robots, int max_obstacles) : fow_angle_(fow_angle), min_distance_(min_distance), max_distance_(max_distance), num_robots_(num_robots), max_obstacles_(max_obstacles)
    {
        ...
        radius = 3.0;
        target_distance_ = max_distance_ * sin(M_PI/(num_robots_+1));           // corda di un cerchio = 2r*sin(th/2). r = R/2, th = 2*pi/(n+1)
        th_des = 2 * M_PI / (num_robots_ + 1);                                   // desired angle between mates to be evenly distributed on a circumference
        ...
        vars_num_ = 4 + num_robots_ + max_obstacles_ + 1;
    ...
    int FormationController::applyCbf(Eigen::Vector3d &uopt, Eigen::Vector3d &ustar, Eigen::MatrixXd &p_js_i, Eigen::MatrixXd &obs_i, Eigen::VectorXd &h_out)
    {
        ...
        for (int i = 0; i < num_robots_; i++)
        {
            ...
            upperbound(4 + i) = gamma_safe_ * pow(h_i, 3);
        }
        ...
        for (int i = 0; i < max_obstacles_; i++)
        {
            ...
            A.insert(4 + num_robots_ + i, 0) = 2 * p_j_i(0);
            A.insert(4 + num_robots_ + i, 1) = 2 * p_j_i(1);
            A.insert(4 + num_robots_ + i, 2) = 0;
            A.insert(4 + num_robots_ + i, 3) = -0.0001;           // slack var
            double h_i = pow(p_j_i.norm(), 2) - pow(min_distance_, 2);
            upperbound(4 + num_robots_ + i) = gamma_safe_ * pow(h_i, 3);
        }
        ...
        // {
        //     Eigen::VectorXd p_j_i = p_js_i.col(i);
        //     A.insert(4 + num_robots_ + max_obstacles_ + i, 0) = -2 * p_j_i(0);
        //     A.insert(4 + num_robots_ + max_obstacles_ + i, 1) = -2 * p_j_i(1);
        //     A.insert(4 + num_robots_ + max_obstacles_ + i, 2) = 0;
        //     A.insert(4 + num_robots_ + max_obstacles_ + i, 3) = -0.0001;           // slack var
        //     double h_i = -pow(p_j_i.norm(), 2) + pow(max_distance_, 2);
        //     upperbound(4 + num_robots_ + max_obstacles_ + i) = gamma_fov_ * h_i;
        // }
        ...
        //     A.insert(4 + num_robots_ + max_obstacles_ + num_robots_ + i, 0) = -K * p_j_i(0);
        //     A.insert(4 + num_robots_ + max_obstacles_ + num_robots_ + i, 1) = -K * p_j_i(1);
        //     A.insert(4 + num_robots_ + max_obstacles_ + num_robots_ + i, 2) = 0;
        //     A.insert(4 + num_robots_ + max_obstacles_ + num_robots_ + i, 3) = -1.0;           // slack var
        //     V = pow(z - target_distance_, 2);
        //     upperbound(4 + num_robots_ + max_obstacles_ + num_robots_ + i) = -gamma_clf_ * V;
        // }

        // CLF for desired distance from center
        Eigen::MatrixXd robots;
        robots.resize(p_js_i.rows(), p_js_i.cols()+1);
        robots.setZero();
        robots.leftCols(p_js_i.cols()) = p_js_i;
        Eigen::VectorXd x_ctr = robots.rowwise().mean();
        double V, K, z;
        z = x_ctr.head(2).norm();
        std::cout << "Dist: " << z << std::endl;
        K = 2 * (z - radius) / z;
        A.insert(4 + num_robots_ + max_obstacles_, 0) = -K * x_ctr(0); 
        A.insert(4 + num_robots_ + max_obstacles_, 1) = -K * x_ctr(1); 
        A.insert(4 + num_robots_ + max_obstacles_, 2) = 0.0; 
        A.insert(4 + num_robots_ + max_obstacles_, 3) = -1.0;
        V = pow(z - radius, 2);
        upperbound(4 + num_robots_ + max_obstacles_) = -gamma_clf_ * V;
        ...
        uopt = u_i.head(3);
        auto hdot = A * u_i;
        std::cout << "epsilon: " << u_i(3) << std::endl;
