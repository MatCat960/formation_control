...
for i in range(0, GRID_SIZE**2, GRID_SIZE):
  ...
  for j in range(0, GRID_SIZE):
    ...
    for target in targets:
        ...
        if dist_t < 2.0:
          rnd = np.random.rand()
          if rnd < 0.5:             # only 50% of detections
            Y[i+j] = 1
...
for s in range(1, NUM_STEPS+1):
  ...
  for i in range(targets.shape[0]):
    new_x = targets[i, 0] + 0.5*vmax * np.cos(thetas[i])
    new_y = targets[i, 1] + 0.5*vmax * np.sin(thetas[i])

    # check if outside 
    if new_x < 1.0:
      new_x = 1.0
      thetas[i] = np.pi - thetas[i] - 0.1*np.pi + 0.2*np.pi*np.random.rand()
    ...
    if new_y < 1.0:
      new_y = 1.0
      thetas[i] = -thetas[i] -0.1*np.pi + 0.2*np.pi*np.random.rand()
  ...
  imgpath = path / f"pics/moving_target/timevar/img{s}.png" if TIME_VAR else path / f"pics/moving_target/fixed/img{s}.png"
  plt.savefig(imgpath)
  # plt.show()
  plt.close()
...
finalpath = path / "pics/moving_target/timevar/final.png" if TIME_VAR else path / "pics/moving_target/fixed/final.png"
